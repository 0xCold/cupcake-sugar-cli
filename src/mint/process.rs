...
pub fn process_mint(args: MintArgs) -> Result<()> {
    ...
    //print_candy_machine_state(candy_machine_state);

    let collection_pda_info =
        Arc::new(get_collection_pda(&candy_pubkey, &client.program(Pubkey::from_str("DsRmdpRZJwagptu4MMN7GJWaPuwPgStWPUSbfAinYCg9").unwrap())).ok());
...
pub fn mint(
    ...
    let program = client.program(Pubkey::from_str("DsRmdpRZJwagptu4MMN7GJWaPuwPgStWPUSbfAinYCg9").unwrap());
